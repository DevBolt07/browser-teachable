...
export function openLabelingModal() {
    ...
    if (!labelingModal.dataset.initialized) {
        ...
        rotateLeftBtn?.addEventListener('click', () => { markUnsaved(); currentTransforms.rotation -= 90; updateCanvasDisplay(); });
        rotateRightBtn?.addEventListener('click', () => { markUnsaved(); currentTransforms.rotation += 90; updateCanvasDisplay(); });
...
function populateLabelingGrid() {
    ...
    if (labelClassFilterSelect) {
        ...
        labelClassFilterSelect.innerHTML = '<option value="">All Classes</option>';
...
function renderGrid() {
    ...
    if (filteredImages.length === 0) {
        labelingGrid.innerHTML = '<div style="grid-column:1/-1; text-align:center; color:#a0aec0; padding:40px;">No images found.</div>';
        return;
    }
...
function resetTransforms() {
    currentTransforms = {
        ...
        saturation: 100,
        rotation: 0,
        filters: { grayscale: 0, blur: 0, invert: 0 }
    };
...
function renderCanvas() {
    ...
    ctx.drawImage(
        ...
        -currentOriginalImage.width / 2,
        -currentOriginalImage.height / 2
    );
...
function drawCropRect(e) {
    ...
    labelingState.cropRect = { x, y, w, h };
    updateCanvasDisplay();
}
...
function applyCrop() {
    ...
    currentOriginalImage.onload = () => {
        ...
        currentTransforms.filters = { grayscale: false, blur: false, sharpen: false, invert: false };
        if (labelBrightnessSlider) labelBrightnessSlider.value = 100;
        if (labelContrastSlider) labelContrastSlider.value = 100;
        if (labelSaturationSlider) labelSaturationSlider.value = 100;
        updateCanvasDisplay();
